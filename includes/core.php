    ...
    public function render($content = "")
    {
        if ($this->themeType == 'frontend') {
            ...
            if ($pageDetails) {
                ...
                if (isset($pageDetails['description'])) $this->setMetaTags(array('description' => $pageDetails['description']));
                if (isset($pageDetails['keywords'])) $this->setMetaTags(array('keywords' => $pageDetails['keywords']));
            }
...
class Uploader
{
    ...
    public function __construct($source, $type = "image", $validate = false, $fromFile = false, $isLink = false)
    {
        ...
        $this->videoTypes = explode(',', config('video-file-types', 'mp4,mov,wmv,3gp,avi,flv,f4v'));
        $this->fileTypes = explode(',', config('files-file-types', 'doc,xml,exe,txt,zip,rar,mp3,jpg,png,css,psd,pdf,3gp,ppt,pptx,xls,xlsx,html,docx,fla,avi,mp4,swf,ico,gif,jpeg'));
