        ...
        if (empty($index)) {
            ...
            foreach($result as $k => $v) {
                if (is_array($v)) {
                    $newResult = array();
                    foreach($v as $n => $a) {
                         $newResult[$n] =  (!is_array($a) and $escape) ?  str_replace("\\\"", "\"", str_replace("\\n", "\n", str_replace("\\r", "\r", mysqli_real_escape_string(db(), sanitizeText($a))))) : $a;
                    }
                    $nR[$k] = $newResult;
                } else {
                    $nR[$k] = ($escape) ?  str_replace("\\\"", "\"", str_replace("\\n", "\n", str_replace("\\r", "\r", mysqli_real_escape_string(db(), $v)))) : $v;
                }
            }
            $result = $nR;
        } else {
            ...
            if (is_array($result[$index])) {
                ...
                foreach($result[$index] as $n => $v) {
                    $newResult[$n] = ((!is_array($v)) and $escape) ? str_replace("\\\"", "\"", str_replace("\\n", "\n", str_replace("\\r", "\r", mysqli_real_escape_string(db(), sanitizeText($v))))) : $v;
                }
                $result = $newResult;
            } else {
                $result = (!is_array($result[$index]) and $escape) ?  str_replace("\\\"", "\"", str_replace("\\n", "\n", str_replace("\\r", "\r", mysqli_real_escape_string(db(), sanitizeText($result[$index]))))) : $result[$index];
            }

        }
    } else {
        $result = (!is_array($result) and $escape) ?  str_replace("\\\"", "\"", str_replace("\\n", "\n", str_replace("\\r", "\r", mysqli_real_escape_string(db(), sanitizeText($result))))) : $result;
    }
