...
function login_user($username, $password, $remember = false) {
    ...
    $tries = session_get("sv_login_tries");
    //exit($tries);
    session_put("sv_login_tries", $tries + 1);
    return false;
}
register_hook('install.require', function() {
    ...
    if ($key) {
        ...
        try {
            $url = "0";
			// "http://crea8social.com/check/key?key=" . $key."&type=".config('type').'&domain='.getHost();
//            exit($url);
            $result = $url;
...
function process_loggedin_user($db) {
    ...
    if (isset($_SESSION['sv_loggin_username']) and isset($_SESSION['sv_loggin_password'])) {
         ...
         */
        if (config('session_timeout', 1800) != 0) {
...
register_hook('install.set', function() {install_languages();installer_plugins();});
...
function find_user($id, $all = true) {
    $whereClause = "";
    //if()
    if (is_numeric($id)) {
        $whereClause = "`id`='{$id}' AND bannned='0'";
    } elseif(preg_match('/@/', $id)) {
        $whereClause = "`email_address`='{$id}' AND bannned='0'";
    } else{
        $whereClause = "`username`='{$id}' AND bannned='0'";
    }
    if ($all) {
        //if ()
 ...
 * @return boolean
 */
function update_user($fields, $userid = null, $reload = false) {
    $userid = ($userid) ? $userid : get_userid();
    $sqlFields = "";
...
function add_custom_field($type, $val, $save = false, $id = null) {
    ...
    if ($save) {
        ...
        $query = db()->query("UPDATE `custom_fields` SET
