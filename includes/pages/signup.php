...
function signup_pager($app) {
    ...
    if ($val) {
        ...
        if (!config("enable-captcha") or (config('enable-captcha') == 1 and strtolower(session_get("sv_captcha")) == strtolower(input('captcha'))) or(config('enable-captcha') == 2 and $resp->isSuccess())) {
            $rules = array(
