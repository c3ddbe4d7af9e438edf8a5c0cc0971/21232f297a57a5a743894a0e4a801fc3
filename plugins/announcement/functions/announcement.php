...
function get_user_announcements($limit = 10, $top = false, $all = false) {
    $time = time();
    if($all) {
        $sql = "SELECT content,time,type,id,time,can_close FROM announcements WHERE active='1' AND start_date < {$time} AND ((user_group='0' OR user_group='100') ";
    } else {
        $ids = implode(',', get_hide_announcements());
        $sql = "SELECT content,time,type,id,time,can_close FROM announcements WHERE active='1' AND start_date < {$time} AND id NOT IN ({$ids}) AND ((user_group='0' OR user_group='100') ";
    }
    if (is_loggedin()) {
        $user = get_user();
        $role = $user['role'];
        ...
        $gender = $user['gender'];
        $sql .= "OR ((user_group='{$role}' OR user_group='0') AND ((location='all' OR location='{$location}') OR (gender='all' OR gender='{$gender}'))) ";
    }
