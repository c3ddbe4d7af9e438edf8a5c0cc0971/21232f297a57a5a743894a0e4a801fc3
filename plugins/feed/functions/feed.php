...
function add_feed($val) {
    ...
    if (empty($content) and empty($images) and empty($files) and empty($music) and empty($video) and empty($feeling_text) and empty($feeling_data) and empty($location)) return json_encode($result);
...
function get_feeds($type = "feed", $type_id = null, $limit = null, $offset = 0, $update = false, $u = true) {
    ...
    elseif($type == 'timeline') {
        ...
        $sql = "SELECT {$sqlFields} FROM `feeds` WHERE (((`entity_id`='{$userid}' AND `entity_type`='user' AND ({$privacy})) OR to_user_id='{$userid}')";
...
function feed_process_link($link) {
    ...
    //exit(var_dump($embed));
    if ($embed) {
        $image = isset($embed->images[0]) ? $embed->images[0] : $embed->image;
        if ($image) {
