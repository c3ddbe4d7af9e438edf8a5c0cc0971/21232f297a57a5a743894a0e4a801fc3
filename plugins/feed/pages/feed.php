...
function feed_pager($app) {
    ...
    return $app->render(view("feed::content", array('feeds' => get_feeds('feed', $type), 'type' => $type)));
}
...
function feed_page_pager($app) {
    ...
    if($feed['shared']) {$title = $feed['shared-feed']['publisher']['name'];} else {$title = $feed['publisher']['name'];};
    $app->setTitle($title);
    $image = null;
    if ($feed['photos']) {
