...
function group_add($val) {
    ...
    $query = db()->query("INSERT INTO groups (group_title,group_name,group_description,user_id,who_can_add_member,privacy)VALUES(
        '{$title}','{$name}','{$description}','{$userid}','{$add}','{$privacy}'
    )");

    $groupId = db()->insert_id;
    //testing for non-english page url
    $slug = toAscii($name);
    if (!preg_match('/^[\pL\pN]+$/u', $name) or empty($slug) or strlen($name) != strlen($slug)) {
        $newSlug = 'page-'.$groupId;
        if ($slug) {
        ...
        db()->query("UPDATE groups SET group_name='{$newSlug}' WHERE group_id='{$groupId}'");
    }
