...
function page_add($val) {
    ...
    $page_url = sanitizeText($page_url);
    $category = sanitizeText($category);
    $query = db()->query("INSERT INTO `pages` (page_user_id,page_title,page_desc,page_url,page_created_at,page_category_id)VALUES(
        '{$userid}','{$name}', '{$description}','{$page_url}','{$time}','{$category}'
    )");

    $insertedId = db()->insert_id;
    //testing for non-english page url

    $slug = toAscii($page_url);
    if (!preg_match('/^[\pL\pN]+$/u', $page_url) or empty($slug) or strlen($page_url) != strlen($slug)) {
        $newSlug = 'page-'.$insertedId;
        if ($slug) {
        ...
        db()->query("UPDATE pages SET page_url='{$newSlug}' WHERE page_id='{$insertedId}'");
    }
    /**
...
function get_invite_friends($term = null, $limit = 20, $offset = 0) {
    $friends = get_friends();
    $friends[] = 0;
