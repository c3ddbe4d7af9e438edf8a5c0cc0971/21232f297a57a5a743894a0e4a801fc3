...
function get_friend_requests($dropdown = false) {
    $userid = get_userid();
    if ($dropdown) {
        $query = db()->query("SELECT * FROM `relationship` INNER JOIN `users` ON relationship.from_userid=users.id WHERE `to_userid`='{$userid}' AND confirm='0' ORDER BY time desc LIMIT 5 ");
        return fetch_all($query);
    } else {
        return paginate("SELECT * FROM `relationship` INNER JOIN `users` ON relationship.from_userid=users.id WHERE `to_userid`='{$userid}' AND confirm='0' ORDER BY time desc ", 10);
    }
}
...
function relationship_suggest($limit, $refId = null) {
    ...
    $query = "SELECT {$fields} FROM `users` WHERE `id`NOT IN({$refId}) AND ({$whereClause}) AND id NOT IN ({$ignoredUsers}) ORDER BY rand()";
    //exit($query);
