...
TaskManager::add('video-processing', function() {
    if (config('video-encoder') == 'ffmpeg') {
        ...
        include_once path("plugins/video/library/FFmpeg.php");
        ...
        while($fetch = $query->fetch_assoc()) {
            ...
            if ($file_path and file_exists(path($file_path))) {
                try{
                    $ffmpeg  = new FFmpeg(config('video-ffmpeg-path'));
                    ...
                    $frames = 10;
                    $size = '230x230';
                    $ffmpeg->input(path($file_path))->thumb( $size , $start, $frames )->output(path($fullPhotoPath))->ready();

                    $details = pathinfo($file_path);
                    $ext = strtolower($details['extension']);
                    $output_file = str_replace($ext, 'mp4', $file_path);
                    $ffmpeg  = new FFmpeg(config('video-ffmpeg-path'));
                    $ffmpeg->input(path($file_path))->output(path($output_file))->ready();
