...
function process_user_save(t, type, typeId) {
    ...
    var s = o.data('status');
    $.ajax({
        url : baseUrl + 'user/save?type=' + type  + '&id=' + typeId + '&status=' + s,
...
function reloadInits() {
    ...
    if($(window).width() > 1000) {
        ...
        if ($(".profile-container").length > 0) {
            ...
            $('.middle-container .right-col-content').stick_in_parent({bottoming:false,parent:$(".middle-container")});
            $('.middle-container .left-col-content').stick_in_parent({bottoming:false,parent:$(".middle-container")});
        }
...
function save_user_profile_cover() {
    ...
    if (i == 0) {
        ...
        toggle_profile_cover_indicator(true);
        $.ajax({
            url : baseUrl + 'user/profile/cover/reposition?pos=' + i+'&width=' + width,
            success: function(data) {
...
$(function() {
    ...
    $(document).on('mouseover', '.preview-card', function() {
        ...
        if (card.length == 0) {
            ...
            $(this).append(card);
            $.ajax({
                url : baseUrl + 'preview/card?type='+ $(this).data('type') + '&id=' + $(this).data('id'),
                success : function(c) {
                    card.html(c);
                }
            })
...
function forumDeleteReply(replyId, threadId){
    $.post(baseUrl + 'forum/ajax?action=post',
        {
            val: true,
            thread_id: threadId,
            id: replyId,
...
function music_set_list_type(type) {
    $.ajax({url: baseUrl + 'music/ajax?action=set_list_type&type=' + type});
}
    ...
    init : function(playlist, options) {
        ...
        this.play.addEventListener('click', function(event) {
            ...
            if(!music.player.playing || music.player.paused){
                ...
                $.ajax({url: baseUrl + 'music/ajax?action=music_played&id=' + music.player.nowPlaying});
        ...
        this.volume.addEventListener('change', function(event) {
            console.log((music.player.volume.value / 100));
...
if (!music.updateDisplay) {
    music.updateDisplay = function (track) {
        ...
        if(typeof music.player.playing !== 'undefined' && music.player.playing) {
            ...
            document.getElementById('music-player-cover').src = baseUrl + track.cover_art.replace(/_%w_/g , '_200_');
            ...
            if(document.getElementById(trackPageCoverId)) {
                document.getElementById(trackPageCoverId).src = baseUrl + track.cover_art.replace(/_%w_/g , '_200_');
            }
            ...
            if(document.getElementById(musicPageDashboard)) {
                $('#' + musicPageDashboard).load(baseUrl + 'music/ajax?action=music_page_dashboard&id=' + music.player.nowPlaying);
            }
            var musicPageComment = 'music-page-comment';
            if(document.getElementById(musicPageComment)) {
                $('#' + musicPageComment).load(baseUrl + 'music/ajax?action=music_page_comment&id=' + music.player.nowPlaying);
            }
...
music.playlist.editor = {
    searchMusic: function(input) {
        ...
        if(str.length >= 3) {
            ...
            $.ajax({
                url: baseUrl + 'music/playlist/editor/search?term=' + str,
                success: function(data) {
...
function people_set_list_type(type) {
    $.ajax({url: baseUrl + 'people/ajax?action=set_list_type&type=' + type});
...
function ads_load_bid_plans(t, type) {
    ...
    if (input.prop('checked')) {
        ...
        s.css('opacity', '0.6');
        $.ajax({
            url : baseUrl + 'ads/load/plan?type=' + type,
            success : function(data) {
                var json = jQuery.parseJSON(data);
                s.html(json.content);
...
function ads_update_plan_description(t) {
    ...
    var id = select.val();
    $.ajax({
        url : baseUrl + 'ads/load/description?id=' + id,
        success: function(data) {
...
function ads_load_page(t) {
    ...
    if (v) {
        $.ajax({
            url : baseUrl + 'ads/load/page?id=' + v,
            success : function(data) {
                var json = jQuery.parseJSON(data);
...
$(function() {
    $(document).on('click', '.ads-click', function() {
        $.ajax({
            url : baseUrl + 'ads/clicked?id=' + $(this).data('id')
        });
    });
    ...
    if (dropdown.css('display') == 'none') {
        ...
        indicator.show();
        $.ajax({
            url : baseUrl + 'chat/load/dropdown',
            success : function(data) {
                content.html(data);
                indicator.hide();
                reloadInits();
...
function change_online_status(type, c) {
    ...
    b.addClass(c);
    $.ajax({
        url : baseUrl + 'chat/set/status?type=' + type
    })
    return false;
}
...
function switch_chat_send_button(e) {
    ...
    $.ajax({
        url : baseUrl + 'chat/update/send/privacy?v=' + v
    })
...
var Chat = {
    ...
    registerBox: function(cid, uid) {
        $.ajax({
            url : baseUrl + 'chat/register/open',
    ...
    unRegisterBox : function(cid) {
        $.ajax({
            url : baseUrl + 'chat/register/open?action=delete',
            data : {cid: cid}
        });
    ...
    open : function(cid, uid, title, img, img2, reg) {
            ...
            if (this.boxCreated(cid, uid)) {
               ...
               $("#chat-boxes-container .chat-boxes-wrapper").append(div);
                    ...
                    if (arBox.length && arBox.data('type') != undefined && arBox.data('type') == 'single') {
                        if ((e.keyCode || e.which ) != 13) {
                            if(arBox.data('typing') == 0) {
                                ...
                                arBox.data('typing', 1);
                                $.ajax({
                                    url : baseUrl + 'chat/typing?cid=' + arBox.data('id')
                                })
                            }
                ...
                textarea.blur(function(e) {
                    ...
                    arBox.data('typing', 0);
                    $.ajax({
                        url : baseUrl + 'chat/remove/typing?cid=' + arBox.data('id')
                    });
                });
    ...
    pushArrived: function() {
        ...
        if (this.toLoad.length > 0) {
            $.ajax({
                url : baseUrl + 'chat/load/messages',
                type : 'POST',
...
function push_chat(type, d) {
    if (type == 'chat') {
        ...
        if (d != '') {
            ...
            if (ids.length > 0) {
                $.ajax({
                    url : baseUrl + 'chat/get/conversations',
                    data : {cids : ids},
                    success : function(data) {
                        var json = jQuery.parseJSON(data);
                        $.each(json, function(cid, v) {
                            if (jQuery.inArray(cid, Chat.cidClosed) == -1) {
                                Chat.open(cid, v.uid, v.title, v.avatar, v.avatar2, false);
                            }
...
function delete_chat_message(id) {
    ...
    $.ajax({
        url : baseUrl + 'chat/delete/message?id=' + id,
        success : function() {
...
function show_more_comment(type, typeId, indicator) {
    ...
    $.ajax({
        url : baseUrl + 'comment/more?type=' + type + '&type_id=' + typeId + '&offset=' + offset + '&limit=' + limit,
        type : 'GET',
        dataType :'html',
        success : function(data) {
...
function show_comment_replies(id, gId) {
    ...
    editor.fadeIn();
    $.ajax({
        url : baseUrl + 'comment/load/replies?id=' + id,
        success : function(data) {
...
$(function () {
    ...
    $(document).on('click', '.emoticon-button', function() {
        ...
        if (e.css('display') == 'none') {
            if (e.html() == '') {
                $.ajax({
                    url : baseUrl + 'emoticon/load?target=' + $(this).data('target') + '&action=' + $(this).data('action'),
                    success : function(data) {
                        e.html(data);
                        reloadInits();
                    }
...
function save_event_profile_cover() {
    ...
    if (i == 0) {
        ...
        toggle_profile_cover_indicator(true);
        $.ajax({
            url : baseUrl + 'event/cover/reposition?pos=' + i + '&id=' + id+'&width=' + width,
            success: function(data) {
                $('.profile-cover-wrapper img').attr('src', data);
                toggle_profile_cover_indicator(false);
                refresh_profile_cover_positioning();
...
function remove_event_profile_cover(img) {
    ...
    var id = $('#event-profile-container').data('id');
    $.ajax({
        url : baseUrl + 'event/cover/remove?id=' + id
    });
    return false;
}
...
function event_invite_friend(t, userid, id) {
    ...
    o.css('opacity', '0.5');
    $.ajax({
        url : baseUrl + 'event/invite/user?id=' + id + '&userid=' + userid,
        success : function(data) {
...
function event_search_invite_friend(i) {
    ...
    if (input.val().length > 1) {
        ...
        $.ajax({
            url : baseUrl + 'event/invite/search?id=' + container.data('id') + '&term=' + input.val(),
            success : function(data) {
                container.html(data);
            }
        })
...
function event_rsvp(t, id) {
    ...
    s.css('opacity', '0.5');
    $.ajax({
        url : baseUrl + 'event/rsvp?id=' + id + '&v=' + s.val(),
        success : function(d) {
...
var feedEditor = {
    ...
    init : function() {
        ...
        $(document).on('click', '.feed-privacy-toggle', function() {
            ...
            $.ajax({
                url: baseUrl + 'feed/update/privacy?id=' + feed + '&privacy=' + p
            })
            return false;
        });
    ...
    listenMediaFeeling: function(t) {
        ...
        if (type == 'watching' || type == 'listening-to') {
            if (i.val().length > 0 ) {
                $.ajax({
                    url : baseUrl + 'feed/search/media?type=' + type + '&term=' + i.val(),
                    success : function(data) {
                        if (data) {
    ...
    processEditorPrivacyDropdown : function() {
        $(document).on('click', "#feed-privacy-dropdown li a", function() {
            ...
            $.ajax({
                url : baseUrl + 'feed/editor/privacy?v=' + $(this).data('id')
            });
        });
    },
...
function toggle_feed_notification(id) {
    ...
    $.ajax({
        url : baseUrl + 'feed/notification?type=' + w+'&id='+id
    });
...
function show_voters(t, answer_id, page) {
    ...
    lists.html('');
    $.ajax({
        url : baseUrl + 'feed/load/voters?answer_id=' + answer_id + '&page=' + page,
        success : function(data) {
            indicator.hide();
            lists.html(data);
...
function save_game_profile_cover() {
    ...
    if (i == 0) {
        ...
        toggle_profile_cover_indicator(true);
        $.ajax({
            url : baseUrl + 'game/cover/reposition?pos=' + i + '&id=' + id+'&width=' + width,
            success: function(data) {
                $('.profile-cover-wrapper img').attr('src', data);
                toggle_profile_cover_indicator(false);
                refresh_profile_cover_positioning();
...
function remove_game_profile_cover(img) {
    ...
    var id = $('#game-profile-container').data('id');
    $.ajax({
        url : baseUrl + 'game/cover/remove?id=' + id
    });
    return false;
}
...
function save_group_profile_cover() {
    ...
    if (i == 0) {
        ...
        toggle_profile_cover_indicator(true);
        $.ajax({
            url : baseUrl + 'group/cover/reposition?pos=' + i + '&id=' + id+'&width=' + width,
            success: function(data) {
                $('.profile-cover-wrapper img').attr('src', data);
                toggle_profile_cover_indicator(false);
                refresh_profile_cover_positioning();
...
function remove_group_profile_cover(img) {
    ...
    var id = $('#group-profile-container').data('id');
    $.ajax({
        url : baseUrl + 'group/cover/remove?id=' + id
    });
    return false;
}
...
function join_group(t) {
    ...
    obj.css('opacity', '0.5');
    $.ajax({
        url : baseUrl + 'group/join?id=' + id + '&status=' + status,
        success : function() {
...
function process_group_role(t, id, uid) {
    ...
    obj.css('opacity', '0.5');
    $.ajax({
        url : baseUrl + 'group/member/role?id=' + id + '&uid=' + uid + '&v=' + obj.val(),
        success : function(data) {
            notifySuccess(data);
            obj.css('opacity', 1);
        }
...
$(function() {
    $(document).on('click', '#group-tags-suggestion a', function() {
        ...
        $.ajax({
            url : baseUrl + 'group/add/member?id=' + id + '&uid=' + uid,
            success : function(data) {
                notifySuccess(data);
            }
        })
    ...
    $.ajax({
        url : baseUrl + 'like/item?type=' + type + '&type_id=' + typeId + '&w=' + w + '&action=like',
        type : 'GET',
        success : function(data) {
            var json = jQuery.parseJSON(data);
...
function dislike_item(type, typeId) {
    ...
    $.ajax({
        url : baseUrl + 'like/item?type=' + type + '&type_id=' + typeId + '&w=' + w + '&action=dislike',
        type : 'GET',
        success : function(data) {
            var json = jQuery.parseJSON(data);
...
function show_likes(type, typeId) {
    ...
    lists.html('');
    $.ajax({
        url : baseUrl + 'like/load/people?type='+type + "&id=" + typeId+"&action=1",
        success : function(data) {
            indicator.hide();
            lists.html(data);
...
function show_dislikes(type, typeId) {
    ...
    lists.html('');
    $.ajax({
        url : baseUrl + 'like/load/people?type='+type + "&id=" + typeId+"&action=0",
        success : function(data) {
            indicator.hide();
            lists.html(data);
...
function show_reactors(t,type, typeId) {
    ...
    lists.html('');
    $.ajax({
        url : baseUrl + 'like/load/people?type='+type + "&id=" + typeId+"&action=3",
        success : function(data) {
            indicator.hide();
            lists.html(data);
...
function react(type, typeId, t) {
    ...
    $.ajax({
        url : baseUrl + 'like/react?type=' +type + '&id=' + typeId + '&code=' + t,
        success : function(data) {
...
;$(function() {
    ...
    if (dropdown.css('display') == 'none') {
        ...
        indicator.show();
        $.ajax({
            url : baseUrl + 'notification/load/latest',
            success : function(data) {
                content.html(data);
                indicator.hide();
                reloadInits();
...
function process_notification_mark(id) {
    ...
    $.ajax({
        url : baseUrl + 'notification/mark?id=' + id + '&type=' + type
    });
    if (status == 0) {
...
function delete_notification(id) {
    ...
    c.fadeOut();
    $.ajax({
        url : baseUrl + 'notification/delete?id=' + id
    });
    return false;
}
...
function push_notification(type, d) {
    if (type == 'notification') {
        ...
        if (nIds) {
            $.ajax({
                url : baseUrl + 'notification/preload',
                data : {ids : nIds},
                success : function(data) {
...
function save_page_profile_cover() {
    ...
    if (i == 0) {
        ...
        toggle_profile_cover_indicator(true);
        $.ajax({
            url : baseUrl + 'page/cover/reposition?pos=' + i + '&id=' + id+'&width=' + width,
            success: function(data) {
                $('.profile-cover-wrapper img').attr('src', data);
                toggle_profile_cover_indicator(false);
                refresh_profile_cover_positioning();
...
function remove_page_profile_cover(img) {
    ...
    var id = $('#page-profile-container').data('id');
    $.ajax({
        url : baseUrl + 'page/cover/remove?id=' + id
    });
    return false;
}
...
function page_invite_friend(l, u, p) {
    ...
    obj.css('opacity', '0.5');
    $.ajax({
        url : baseUrl + 'page/invite/friend?page=' + p + '&user=' + u,
        success : function(data) {
            obj.remove();
        },
        error : function() {
...
function page_search_invite_friend(i) {
    ...
    if (input.val().length > 1) {
        $.ajax({
            url : baseUrl + 'page/invite/search?page=' + container.data('id') + '&term=' + input.val(),
            success : function(data) {
                container.html(data);
            }
        })
...
function page_hook_page_loaded() {
    ...
    container.slimScroll().bind('slimscroll', function(e, pos) {
        if (pos == 'bottom') {
            $.ajax({
                url : baseUrl + 'page/more/invite?offset=' + container.data('offset') + '&id=' + container.data('id'),
                success : function(data) {
                    var json = jQuery.parseJSON(data);
...
function paginate_album_photos() {
    ...
    $.ajax({
        url : baseUrl + "photo/album/photos/paginate",
...
function paginate_photo_albums() {
    ...
    window.paginatingAlbumPhotos = true;
    $.ajax({
        url : baseUrl + "photo/album/paginate",
...
$(function() {
    ...
    $.ajax({
        url : baseUrl + 'relationship/follow?type=' + type + '&userid=' + userid
    })
    return false;
}
...
function process_friend(userid) {
    ...
    if (status == 0) {
        ...
        $.ajax({
            url : baseUrl + 'relationship/add/friend?userid=' + userid,
            success : function(data) {
                if (data == 1) {
        ...
        confirm.action(function() {
            o.css('opacity', '0.4');
            $.ajax({
                url : baseUrl + 'relationship/remove/friend?userid=' + userid,
                success: function(data) {
                    if (data == 1) {
...
function show_friend_request_dropdown() {
    ...
    if (dropdown.css('display') == 'none') {
        ...
        indicator.show();
        $.ajax({
            url : baseUrl + 'relationship/load/requests',
            success : function(data) {
                content.html(data);
                indicator.hide();
            }
...
function confirm_friend_request(userid, b) {
    ...
    $.ajax({
        url : baseUrl + 'friend/request/confirm?userid=' + userid,
        type : 'GET',
        success : function(data) {
            if (data == 'login') {
                login_required();
...
function push_friend_requests(type, d) {
    if (type == 'friend-request') {
        ...
        if (nIds) {
            $.ajax({
                url : baseUrl + 'friend/requests/preload',
                data : {ids : nIds},
                success : function(data) {
...
function process_search_dropdown() {
    ...
    if (input.val().length > 1) {
        ...
        fullSearch.prop('href', baseUrl + 'search?term=' + encodeURIComponent(input.val()));
...
function social_invite_user(t) {
    ...
    $.ajax({
        url : baseUrl + 'social/invite/user?email=' + email,
        success : function(d) {
            button.fadeOut();
        }
    });
